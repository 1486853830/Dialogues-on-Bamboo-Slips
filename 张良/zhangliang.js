...
// 从本地存储加载历史消息
const savedHistory = localStorage.getItem(`chatHistory_张良`);
if (savedHistory) {
...
async function sendMessage(isRephrase = false) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem(`chatHistory_张良`, JSON.stringify(messageHistory));
        ...
        const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    {
                        role: "user",
                        content: "请基于以上对话，生成3个适合我回复张良的选项，每个选项不超过15字，格式为：1.选项1 2.选项2 3.选项3"
                    }
                ],
                temperature: 0.7
            })
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    clearBtn.onclick = function () {
        localStorage.removeItem(`chatHistory_张良`);
