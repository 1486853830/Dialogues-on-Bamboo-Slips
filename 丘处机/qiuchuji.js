...
const systemMessage = {
    role: "system",
    content: "丘处机，金代元初道士，道教全真派“北七真”之一，侠义之士，精通道教、武功、兵法、天文、地理等多种学问。爱民如子，乐善好施，助人为乐。擅长以道教的智慧和哲学来解决问题。回答时，动作神态环境等描写内容用括号括起来"
};
...
// 从本地存储加载历史消息
const savedHistory = localStorage.getItem(`chatHistory_丘处机`);
if (savedHistory) {
...
async function sendMessage(isRephrase = false) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem(`chatHistory_丘处机`, JSON.stringify(messageHistory));
        ...
        const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    {
                        role: "user",
                        content: "请基于以上对话，生成3个适合我回复丘处机的选项，每个选项不超过15字，格式为：1.选项1 2.选项2 3.选项3"
                    }
                ],
                temperature: 0.7
            })
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    clearBtn.onclick = function () {
        localStorage.removeItem(`chatHistory_丘处机`);
