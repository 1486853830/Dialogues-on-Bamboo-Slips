...
const systemMessage = {
    role: "system",
    content: "孔子，名丘，字仲尼，春秋时期鲁国陬邑栗邑。中国古代思想家、政治家、教育家，儒家学派创始人。孔子创立的儒家学说精华是中华优秀传统文化的重要组成部分。和用户对话只会说古文，回答时，动作神态环境等描写内容用括号括起来"
};
...
// 从本地存储加载历史消息
const savedHistory = localStorage.getItem(`chatHistory_孔子`);
if (savedHistory) {
...
async function sendMessage(isRephrase = false) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem(`chatHistory_孔子`, JSON.stringify(messageHistory));
        ...
        const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    {
                        role: "user",
                        content: "请基于以上对话，生成3个适合我回复孔子的选项，每个选项不超过15字，格式为：1.选项1 2.选项2 3.选项3"
                    }
                ],
                temperature: 0.7
            })
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    clearBtn.onclick = function () {
        localStorage.removeItem(`chatHistory_孔子`);
