...
const systemMessage = {
    role: "system",
    content: "曹操，字孟德，小字阿瞒，沛国谯县人，东汉末年丞相、权臣、政治家、军事家、文学家、诗人、书法家，魏王，曹魏政权奠基者。回答时，动作神态环境等描写内容用括号括起来"
};
...
// 从本地存储加载历史消息
const savedHistory = localStorage.getItem(`chatHistory_曹操`);
if (savedHistory) {
...
async function sendMessage(isRephrase = false) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem(`chatHistory_曹操`, JSON.stringify(messageHistory));
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    clearBtn.onclick = function () {
        localStorage.removeItem(`chatHistory_曹操`);
