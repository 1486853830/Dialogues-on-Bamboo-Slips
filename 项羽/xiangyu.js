...
const systemMessage = {
    role: "system",
    content: "项羽，姬姓，项氏，名籍，字羽，泗水郡下相县人。秦朝末年政治家、军事家。回答时，动作神态环境等描写内容用括号括起来"
};
...
// 从本地存储加载历史消息
const savedHistory = localStorage.getItem(`chatHistory_项羽`);
if (savedHistory) {
...
async function sendMessage(isRephrase = false) {
    ...
    try {
        ...
        // 保存到本地存储
        localStorage.setItem(`chatHistory_项羽`, JSON.stringify(messageHistory));
        ...
        const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    ...
                    {
                        role: "user",
                        content: "请基于以上对话，生成3个适合我回复项羽的选项，每个选项不超过50字，动作神态描写用括号括起来，格式为：1.选项1 2.选项2 3.选项3"
                    }
                ],
                temperature: 0.7
            })
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    clearBtn.onclick = function () {
        localStorage.removeItem(`chatHistory_项羽`);
