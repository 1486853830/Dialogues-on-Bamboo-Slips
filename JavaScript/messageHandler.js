...
export function displayMessage(chatContainer, message, sender, messageIdCounter, characterName = "默认", autoSpeak = false) {
    ...
    if (sender === 'bot') {
        ...
        if (autoSpeak) {
            try {
                synthesizeSpeech(message, characterName);
            } catch (error) {
                console.error('开场白自动语音失败:', error);
            }
        }
    }
